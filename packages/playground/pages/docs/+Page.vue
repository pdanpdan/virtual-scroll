...
// --- Sticky "On this page" popup (pattern: headless-components playground) --
// The popup lists every h2/h3/h4 heading of the page; the current section is
// highlighted with a scroll spy. The headless-components reference uses the
// CSS-only `:target-current` scrollspy (Chrome 140+); a rAF scroll spy is
// used here so the active state works in every browser.
interface TocItem {
  id: string;
  text: string;
  level: 2 | 3 | 4;
...
/** Centers the active link inside the popup list when the popup opens. */
...
function syncTocActive() {
  ...
  // mobile navbar plus the popup itself); the last one in document order to
...
function refreshTocItems() {
  const root = pageContentRef.value;
  if (!root) {
    return;
  }
  const items: TocItem[] = [];
  const targets = new Map<string, Element>();
  root.querySelectorAll<HTMLAnchorElement>('h2 > a[href^="#"], h3 > a[href^="#"], h4 > a[href^="#"]').forEach((link) => {
...
<template>
  ...
  <div ref="pageContentRef" class="space-y-8">
    <!-- On this page: sticky popup under the title card -->
    <div
      v-if="tocItems.length > 0"
      class="toc-popup sticky top-16 lg:top-0 z-20"
    >
      <details
        ref="tocDetailsRef"
...
<style scoped>
/* --- Sticky "On this page" popup --- */

/* While pinned to the scrollport top, square the popup's top corners against
   the viewport edge (Chrome 133+ scroll-state container query; gracefully
   rounded elsewhere). */
.toc-popup {
...
/* Hash-jumped sections stay clear of the sticky mobile navbar and popup. */
[id] {
